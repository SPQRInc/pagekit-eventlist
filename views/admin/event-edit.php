<?php $view->script('admin-event-edit',
    'spqr/eventlist:app/bundle/admin/event-edit.js', [
        'vue',
        'editor',
        'uikit',
    ]) ?>

<form id="event" class="uk-form" v-validator="form"
      @submit.prevent="save | valid" v-cloak>
	<div class="uk-margin uk-flex uk-flex-space-between uk-flex-wrap"
	     data-uk-margin>
		<div data-uk-margin>
			<h2 class="uk-margin-remove"
			    v-if="event.id">{{ 'Edit Event' | trans }}</h2>
			<h2 class="uk-margin-remove" v-else>{{ 'Add Event' | trans }}</h2>
		</div>
		<div data-uk-margin>
			<a class="uk-button uk-margin-small-right"
			   :href="$url.route('admin/eventlist/event')">{{ event.id ? 'Close' : 'Cancel' | trans }}</a>
